...
cc_config ......... Path to the 'cookiecutterc.yml' file with preconfigured values.
                    Uses .cookiecutter.json if not specified.
                    Value=$cc_config
...
[[ -n "${template_dir}" ]] || err_exit "Template subdirectory name must be specified."
[[ -n "${conda_env}" ]] || err_exit "Conda environment name must be specified."
...
if [[ -f ${cc_config} ]]; then
	...
	cc_config="${root_dir}/tests/integration/fixtures/cookiecutterc_${template_dir}.yml"
fi
[[ -f "${cc_config}" ]] || err_exit "Cookiecutter config file not found: ${cc_config}"
