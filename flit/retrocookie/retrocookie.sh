...
# flit/retrocookie/retrocookie.sh --help
...
Examples:
 ...
 Regenerate to current directory using a different repo and branch, also conda env.
   > ./$script_file -d flit -n cookies path/to/workflow_name
...
cc_config="${cc_config:-${root_dir}/${template_dir}/cookiecutterc.yml}"
...
echo "command: cookiecutter --overwrite-if-exists --no-input --output-dir=${script_pdir}/tmp  --config-file=${cc_config} --directory=${template_dir} ${root_dir}"
...
create_commits() {
	...
	rsync -av --progress "${project_folder}" "${tmp_proj_dir}"/.. --exclude .git --exclude .ipynb_checkpoints
